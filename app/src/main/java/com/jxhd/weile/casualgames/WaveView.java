...
public class WaveView extends View {
    ...
    private static float mMaxRadius;   // 最大波纹半径
    private long mDuration = 2000; // 一个波纹从创建到消失的持续时间
    private int mSpeed = 2600;   // 波纹的创建速度，每500ms创建一个
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (iterator.hasNext()) {
            ...
            if (System.currentTimeMillis() - circle.mCreateTime < mDuration) {
                ...
                canvas.drawCircle(this.minWH / 2, this.minWH / 2, circle.getCurrentRadius(), mPaint);
            } else {
                iterator.remove();
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int mWidth = measureSize(widthMeasureSpec);
        int mHeight = measureSize(heightMeasureSpec);
        minWH = Math.min(mWidth, mHeight);
        this.mMaxRadius = minWH / 2;
        setInitialRadius(minWH/8);
